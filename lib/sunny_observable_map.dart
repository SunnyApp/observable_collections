...
class SunnyObservableMap<K, V> extends ObservableMap<K, V> with LoggingMixin {
  ...
  MapDiffs<K, V> newBuilder() => MapDiffs.builder({...this.safeCast()}, valueEquality: _diffEquality, checkValues: true);
  ...
  R _buildChanges<R>(R builder(MapDiffs<K, V> _)) {
    final changes = MapDiffs<K, V>.builder(this.safeCast(), valueEquality: _diffEquality, checkValues: true);
  ...
  void addAll(Map<K?, V> other) {
    final combined = {...this.safeCast<K, V>(), ...other.cast<K, V>()};
    this.sync(combined);
  }
  ...
  void updateAll(V update(K? key, V value)) {
    final values = {...this.safeCast<K, V>()};
